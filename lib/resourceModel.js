...
exports.list = function(page, size) {
    ...
    Resource.find(query, projection, opts)
            .paginate(page, size, function (error, ResourceObjects, total) {
                if (error) return promise.reject(error, true);
                promise.resolve(ResourceObjects, total);
            });
